...
7. **STAR + 适配总结**：每条 bullet 都要包含「场景/问题 + 行动 + 结果/产出 + 能力总结」。每条中文 bullet 尽量写到 70-95 个汉字，在 A4 预览里约一行半到两行；不要短于 55 个汉字，也不要写成三行以上的大段话。页面整体要呈现信息密集、经验丰富的观感。最后半句必须回归到目标岗位需要的能力，如用户洞察、需求拆解、数据复盘、跨部门推进、商业判断、项目闭环等；不要写"这证明了..."，而是自然接在句尾。
8. **量化优先**：能用数字就用数字；没有数字时写清动作、对象、方法和产出，不要写空泛品质。
9. **高密度一页**：中文简历必须占满 A4 一页，这是硬指标。参考正式求职简历的高信息密度，教育 1-3 行，实习/项目/创业校园经历合计 6-8 段，bullet 合计 22-28 条。不要生成半页简历，也不要让版面显得空。
10. **填充优先级**：经历事实足够时，只写教育、实习经历、项目经历、创业/校园经历。优先扩写过往经历，尤其是实习经历；如果最相关经历不足以铺满页面，必须继续从原始事实池中选择其他项目、创业、校园、比赛、作品经历补进来，并把它们包装成目标岗位可迁移能力，不能因为相关性较弱就删掉。只有原始素材确实极少、无法形成完整一页时，才允许补充「工作技能」和「个人概述」。技能/个人评价永远放最后，不能作为常规模块输出。
...
6. 如果用户要求新增公司、时间、奖项、数据、项目结果、链接等上下文里没有的事实，不要改写原文；请只返回一行：NEED_MORE_FACTS: 请补充……
7. 返回改写后的纯文本（不要 JSON），可以直接替换到原位置
...
返回 JSON：
{
  ...
  window.AIService = {
    ...
    async generateResume({ jd, experiences, requirements = '', previousContent = null }) {
      ...
      userMessage += `\n\n## 一页纸硬性要求\n- 如果用户素材不少于 100 字，初稿就必须接近完整 A4 一页，不要只写半页。\n- 实习/项目/创业校园 bullet 合计不少于 22 条；素材明显很少时至少 16 条，并补充技能和个人概述。\n- 如果最相关经历不够，必须从事实池中继续提取其他经历写入项目经历、创业经历或校园经历；不要只写一两段核心经历后空着页面。\n- 优先展示教育经历、实习经历、项目经历、创业/校园经历；技能和个人概述放最后。\n- 每个 bullet 尽量 70-95 个汉字，在预览区约一行半到两行；太短会显得经历不足，必须补足动作、对象、方法、产出和能力归纳。\n- 每个 bullet 结尾必须有能力归纳，但不要出现第一人称和目标岗位名称。`;
